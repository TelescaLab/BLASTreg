...
#' @param X.test matrix
#' @param y.test vector
#' @param total.step integer
#' @param selection logical
#'
#' @return list
#' @export
...
#' @param n.vec vector of sample sizes
#' @param lam.const lambda constant
#' @param l1 logical indicating l1 or l0 method
#'
#' @return list
#' @export
...
#' @export
#'
mse.fun<- function(est, beta = NULL, X.test=NULL, y.test=NULL){
  pred.err<-NA
  est.err<-NA
  y_hat <- NA
  if(!is.null(beta)){
  ...
  if(!is.null(X.test)){
    y_hat <- X.test%*%est
    pred.err<-  mean((y.test - (y_hat))^2)
  }
  return(list(est.err=est.err, pred.err= pred.err, y_hat=y_hat))
}
